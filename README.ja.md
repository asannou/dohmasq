...
*   **EDNS0 サポート:** クライアントクエリに含まれる `OPT` レコードなどの付加情報（Additional Section）をレスポンスでも維持し、`curl` や Firefox などの厳格なクライアントとの互換性を確保します。
*   **DNS 構造の整合性:** クエリの Question セクションをビット単位で正確に複製し、解決対象ドメインへの非 A レコードクエリ（AAAA 等）に対して適切に応答（回答 0 件の `NOERROR`）を返します。
